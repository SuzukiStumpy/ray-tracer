...
class TestMatrix:
    ...
    def test_invertible_matrix_for_invertibility(self) -> None:
        a = Matrix([[6, 4, 4, 4], [5, 5, 7, 6], [4, -9, 3, -7], [9, 1, 7, -6]])

        assert a.det() == -2120
        assert a.is_invertible()

    def test_non_invertible_matrix_for_invertibility(self) -> None:
        a = Matrix([[-4, 2, -2, -3], [0, 6, 2, 6], [0, -5, 1, -5], [0, 0, 0, 0]])

        assert a.det() == 0
        assert not a.is_invertible()

    def test_calculating_inverse_of_a_matrix(self) -> None:
        a = Matrix([[-5, 2, 6, -8], [1, -5, 1, 8], [7, 7, -6, -7], [1, -3, 7, 4]])
        b = a.inverse()

        assert math.isclose(a.det(), 532, rel_tol=EPSILON)
        assert math.isclose(a.cofactor(2, 3), -160, rel_tol=EPSILON)
        assert math.isclose(b[3, 2], -160 / 532, rel_tol=EPSILON)
        assert math.isclose(a.cofactor(3, 2), 105, rel_tol=EPSILON)
        assert math.isclose(b[2, 3], 105 / 532, rel_tol=EPSILON)
        assert b == Matrix(
            [
                [0.21805, 0.45113, 0.24060, -0.04511],
                [-0.80827, -1.45677, -0.44361, 0.52068],
                [-0.07895, 0 - 0.22368, -0.05263, 0.19737],
    ...
    def test_robustness_test_for_matrix_inversion(
        self, A: Matrix, expected: Matrix
    ) -> None:
        assert A.inverse() == expected

    def test_multiplying_a_product_by_its_inverse_yields_original(self) -> None:
