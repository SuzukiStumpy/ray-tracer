...
class TestTransformations:
    class TestTranslation:
        def test_multiplying_by_a_translation_matrix(self) -> None:
            t: Matrix = Transforms.translation(5, -3, 2)
            p = Point(-3, 4, 5)
    ...
    class TestScaling:
        def test_scaling_matrix_applied_to_a_point(self) -> None:
            t: Matrix = Transforms.scaling(2, 3, 4)
            p = Point(-4, 6, 8)
        ...
        def test_reflection_is_scaling_by_a_negative_value(self) -> None:
            t: Matrix = Transforms.scaling(-1, 1, 1)
            p = Point(2, 3, 4)
    ...
    class TestRotation:
        def test_rotating_a_point_around_the_x_axis(self) -> None:
            p = Point(0, 1, 0)
            half_quarter: Matrix = Transforms.rotation_x(math.pi / 4)
            full_quarter: Matrix = Transforms.rotation_x(math.pi / 2)
        ...
        def test_inverse_rotation_rotates_opposite_direction(self) -> None:
            p = Point(0, 1, 0)
            half_quarter: Matrix = Transforms.rotation_x(math.pi / 4)
            inv: Matrix = half_quarter.inverse()

            assert inv * p == Point(0, math.sqrt(2) / 2, -math.sqrt(2) / 2)

        def test_rotating_a_point_around_the_y_axis(self) -> None:
            p = Point(0, 0, 1)
            half_quarter: Matrix = Transforms.rotation_y(math.pi / 4)
            full_quarter: Matrix = Transforms.rotation_y(math.pi / 2)
        ...
        def test_rotating_a_point_around_the_z_axis(self) -> None:
            p = Point(0, 1, 0)
            half_quarter: Matrix = Transforms.rotation_z(math.pi / 4)
            full_quarter: Matrix = Transforms.rotation_z(math.pi / 2)
    ...
    class TestCompoundTransformations:
        def test_individual_transformations_applied_in_sequence(self) -> None:
            p = Point(1, 0, 1)

            a: Matrix = Transforms.rotation_x(math.pi / 2)
            b: Matrix = Transforms.scaling(5, 5, 5)
            c: Matrix = Transforms.translation(10, 5, 7)
        ...
        def test_chained_transforms_must_be_applied_in_reverse(self) -> None:
            p = Point(1, 0, 1)
